...
<center>
<h1 style="color:red">文章修改<span style="float:right"><a class="btn btn-default" href="{{url('/article')}}">列表</a></span></h1>
</center>
<hr/>
<!-- @if($errors->any())
...
@endif -->
<form class="form-horizontal" role="form" method="post" action="{{url('/article/update/'.$arr->article_id)}}" id="Myform" enctype="multipart/form-data">
@csrf
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">文章标题</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" style="width:900px" name="article_title" id="firstname" 
				   placeholder="请输入文章标题" value="{{$arr->article_title}}" article_id="{{$arr->article_id}}">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			        <select class="form-control" style="width:900px" name="cate_id" id="lastname">
                        ...
                        <option value="{{$v->cate_id}}" @if($v->cate_id==$arr->cate_id) selected @endif>{{$v->cate_name}}</option>
						@endforeach
			        </select>
	...
	<div class="form-group">
		...
		@else
		    <input type="radio" name="order_article" id="lastname" value="0" >普通
			<input type="radio" name="order_article" id="lastname" value="1" checked>置顶
        @endif 
		</div>
	</div>
	<div class="form-group">
		...
		@else
            <input type="radio"  name="is_show" id="lastname" value="0" >显示
			<input type="radio"  name="is_show" id="lastname" value="1"  checked>不显示
		@endif 
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" style="width:900px" name="article_man" id="firstname" 
				   placeholder="请输入文章作者" value="{{$arr->article_man}}">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" style="width:900px" name="article_email" id="firstname" 
				   placeholder="请输入作者email" value="{{$arr->article_email}}">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<input type="text" class="form-control" style="width:900px" name="article_key" id="firstname" 
				   placeholder="关键字" value="{{$arr->article_key}}">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<textarea name="article_desc" style="width:600px;height:150px" rows="" cols="">{{$arr->article_desc}}</textarea>
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">上传文件</label>
		<div class="col-sm-3">
			<input type="file" class="form-control"  name="article_img" id="lastname">
		</div>
		 @if($arr->article_img)
			<img width="50" src="{{env('UPLOADS_URL')}}{{$arr->article_img}}">
		@endif
	</div>
	<div class="form-group">
...
$('input[name="article_title"]').blur(function(){
  ...
  if(!(/^[\u4e00-\u9fa5}\w]{2,60}$/.test(article_title))){
   ...
   $.post('/article/updateName',{article_id:article_id,article_title:article_title},function(res){
...
$("button").click(function(){
   ...
   var article_id = $('input[name="article_title"]').attr('article_id');
  ...
  if(!(/^[\u4e00-\u9fa5}\w]{2,60}$/.test(article_title))){
   ...
   $.ajax({
	...
	data:{article_id:article_id,article_title:article_title},

    url:'/article/updateName',
