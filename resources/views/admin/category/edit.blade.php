...
@endif -->
<form class="form-horizontal" role="form" method="post" action="{{url('/category/update/'.$res->cate_id)}}" >
@csrf
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">分类名称</label>
		<div class="col-sm-10">
			<input type="text" class="form-control" style="width:900px" name="cate_name" id="firstname" 
				   placeholder="请输入品牌名称" value="{{$res->cate_name}}" cate_id="{{$res->cate_id}}">
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			        <select class="form-control" name="pid" id="lastname">
						...
						@foreach($cate as $v)
                        <option value="{{$v->cate_id}}" @if($v->pid==$res->pid)'selected'@else''@endif>{{str_repeat('|——',$v->level)}}{{$v->cate_name}}</option>
						@endforeach
			        </select>
	...
	<div class="form-group">
		...
		<div class="col-sm-10">
			<textarea name="cate_desc" rows="" cols="">{{$res->cate_desc}}</textarea>
		</div>
	</div>
	<div class="form-group">
...
$('input[name="cate_name"]').blur(function(){
  ...
  if(!(/^[\u4e00-\u9fa5}\w]{2,60}$/.test(cate_name))){
   ...
   $.post('/category/updateNameb',{cate_id:cate_id,cate_name:cate_name},function(res){
...
$('select[name="pid"]').blur(function(){
 ...
 $("button").click(function(){
   ...
   var cate_id = $('input[name="cate_name"]').attr("cate_id");
